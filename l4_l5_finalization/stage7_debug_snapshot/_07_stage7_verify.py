...
def process_candidate(
    ...
    if c05 is not None and not c05.passed:
        all_conditions.append(c05)
    if c06 is not None and not c06.passed:
...
def load_cell_tasks_from_bank(bank_data: List[Dict], target_cell_key: str) -> List[Dict]:
    """Load existing tasks from bank for the same cell."""
    return [t for t in bank_data if t.get("cell_key", "") == target_cell_key]


def get_cell_info_from_bank(bank_data: List[Dict], cell_key: str) -> Dict:
    """Extract cell metadata (theme, subtopic, level) from bank entries."""
    for entry in bank_data:
        if entry.get("cell_key") == cell_key:
            return {
    ...
    return {"grade": "", "level": "", "theme_id": "", "theme_name": "", "subtopic": ""}
...
def main():
    ...
    for slot_key, candidate in candidates.items():
        ...
        if not cell_key:
            # Try to build cell_key from slot_key
            cell_key = slot_key.rsplit("|", 1)[0] if "|" in slot_key else slot_key
        cell_info = get_cell_info_from_bank(bank_data, cell_key)
        cell_tasks = load_cell_tasks_from_bank(bank_data, cell_key)
