...
#!/usr/bin/env python3
"""
FORMYLA L4-L5: АВТОНОМНЫЙ ФИКС (финальная версия)
=================================================
- Сохраняет прогресс после каждой задачи
- Если интернет упал — ждёт и пробует снова
- При перезапуске — продолжает с места
- Не пропускает задачи
- 5 ретраев на задачу, потом идёт дальше
- БЕЗ багов с dict & set

Запуск: python formyla_final.py
"""

import json, subprocess, time, re, threading, os
...
def api_call(messages, max_tokens=1000, thinking="disabled", timeout=50):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            r = subprocess.run(
                ['curl', '-sS', '--http1.1', '--max-time', str(timeout),
                 '--connect-timeout', '15', '-X', 'POST', API_URL,
                 '-H', 'Content-Type: application/json',
                 '-H', f'Authorization: Bearer {API_KEY}', '-d', body],
...
def main():
    print('=' * 50)
    print('FORMYLA L4-L5: ФИНАЛЬНЫЙ ФИКС')
