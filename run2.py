#!/usr/bin/env python3
"""
FORMYLA: аудит + фикс базы задач. DeepSeek V4 Pro. 5 потоков.

ЗАПУСК:
    python -u run2.py            # аудит, затем фикс
    python -u run2.py audit
    python -u run2.py fix
    python -u run2.py stats
    python -u run2.py test       # только проверка связи с API

Отличие от run.py: при старте выполняется самопроверка API.
Если аккаунт не принимает thinking / reasoning_effort / response_format,
эти параметры автоматически отключаются, и прогон идёт дальше.
Первые ошибки печатаются полностью, а не прячутся за счётчик ERR.
"""
...
AUDIT_LIMIT = 0
FIX_LIMIT = 0
SHOW_FIRST_ERRORS = 3     # сколько первых ошибок печатать целиком

# определяется автоматически в api_selftest()
...
def api_selftest():
    ...
    # 1. базовый вызов без всяких опций
    try:
        r = raw_call(probe, thinking=False, json_mode=False, model=MODEL_NAME)
        if r.status_code >= 400:
            print(f"  {MODEL_NAME}: HTTP {r.status_code}")
            print(f"  {r.text[:400]}")
            if r.status_code in (401, 402, 403):
                sys.exit("Проблема с ключом или балансом. Прогон невозможен.")
            r2 = raw_call(probe, thinking=False, json_mode=False, model=FALLBACK_MODEL)
            if r2.status_code < 400:
                CAP["model"] = FALLBACK_MODEL
                print(f"  переключаюсь на {FALLBACK_MODEL}")
            else:
                print(f"  {FALLBACK_MODEL}: HTTP {r2.status_code} {r2.text[:300]}")
                sys.exit("Ни одна модель не отвечает.")
        else:
    ...
    # 2. json_mode
    r = raw_call(probe, thinking=False, json_mode=True, model=m)
    CAP["json_mode"] = r.status_code < 400
    print(f"  response_format json_object: {'OK' if CAP['json_mode'] else 'НЕ ПОДДЕРЖИВАЕТСЯ'}")
    if not CAP["json_mode"]:
        print(f"    {r.text[:300]}")

    # 3. thinking
    r = raw_call(probe, thinking=True, json_mode=CAP["json_mode"], model=m)
    CAP["thinking"] = r.status_code < 400
    print(f"  thinking + reasoning_effort: {'OK' if CAP['thinking'] else 'НЕ ПОДДЕРЖИВАЕТСЯ'}")
    if not CAP["thinking"]:
        print(f"    {r.text[:300]}")

    print(f"Итог: модель={CAP['model']}, thinking={CAP['thinking']}, "
          f"json_mode={CAP['json_mode']}\n", flush=True)


# ==================== ЗАПРОС ====================

def request_once(messages, thinking):
    r = raw_call(messages, thinking and CAP["thinking"], CAP["json_mode"], CAP["model"])
...
def stage_audit():
    ...
    if stop.is_set():
        print("\nВСЕ ЗАПРОСЫ ПАДАЮТ. Прогон остановлен, смотри текст ошибки выше "
              "и audit_raw.log.")
        return
...
def stage_fix():
    ...
    print(f"Скопировано без изменений: {len(clean)}")
...
def stage_stats():
    ...
    print(f"Аудировано   : {len(ok)} (строк с ошибкой {len(aud) - len(ok)})")
...
def main():
    ...
    print(f"Модель: {MODEL_NAME} | потоков={WORKERS} | max_tokens={MAX_TOKENS}")
    print(f"База  : {IN_DB}\n")
    ...
    if what == "test":
        print("Самопроверка пройдена. Запусти без аргумента для полного прогона.")
        return
