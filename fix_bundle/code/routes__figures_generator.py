...
_BASE_PLANNER_PROMPT_VERSION = "base-planner-v6"
...
def _run_base_only_job(job_id: int, job) -> None:
    ...
    if cache_key and cache_key in FIGURE_PLAN_CACHE:
        ...
        for attempt in range(max_attempts):
            ...
            # CH-fidelity: нормализация base-плана (incircle_touch вместо
            # point_on_segment с произвольным ratio).
            from services.base_normalizer import normalize_base_plan
            plan = normalize_base_plan(plan)
...
def _run_solver_aux_job(job_id: int, job) -> None:
    ...
    from services.figure_plan_schemas import parse_base_plan, parse_audit_result
    from services.condition_coverage import check_condition_coverage
    from services.solution_generator import solve_problem, repair_solver_aux, SolverError
    ...
    for attempt in range(1 + MAX_REPAIR_ATTEMPTS_BASE_ONLY):
        ...
        if not validate_figure_json(plan).get("valid"):
            ...
            _fail_job(job, "Модель не смогла создать корректный base-план.")
            return
        # CH-fidelity: нормализация base-плана — точки касания вписанной
        # окружности должны быть нативным incircle_touch (не point_on_segment
        # с произвольным ratio), иначе equal_segments_mark «равные отрезки»
        # численно не сойдутся.
        from services.base_normalizer import normalize_base_plan
        plan = normalize_base_plan(plan)
        base_plan = plan
        job.base_model = model
    ...
    # ── aux_compile (с fidelity-проверкой и repair-прогоном) ──
    # CH-fidelity: если компилятор потерял часть объявленных построений
    # (UNKNOWN_AUX_OP / QUOTE_NOT_IN_SOLUTION / MISSING_FIELDS), переспрашиваем
    # решателя один раз, чтобы чертёж выводился ТОЧЬ-В-ТОЧЬ как в решении.
    ...
    fidelity = fidelity_report(solver_result, base_plan)
    job.aux_fidelity = fidelity.get("ratio")
    _record_stage(job_id, "aux_compile",
                  ...
                  coverage_score=fidelity.get("ratio"))

    # ── aux_repair: повторный прогон при потерях ──
    aux_source = "solver"
    if fidelity.get("dropped", 0) > 0:
        _set_stage(job, "aux_repair")
        _t_repair = time.perf_counter()
        repaired = repair_solver_aux(
            condition_text,
            fidelity=fidelity,
            previous=solver_result,
            base_plan=base_plan,
            logger=logger,
        )
        if repaired is not None:
            _usage_r = (repaired or {}).get("_usage", {}) or {}
            _record_stage(
                job_id, "aux_repair", role="solver",
                provider=repaired.get("_provider"),
                model=repaired.get("_model"),
                input_tokens=_usage_r.get("prompt_tokens"),
                output_tokens=_usage_r.get("completion_tokens"),
                reasoning_tokens=_usage_r.get("reasoning_tokens"),
                latency_ms=int((time.perf_counter() - _t_repair) * 1000),
                estimated_cost_usd=repaired.get("_cost_usd"),
            )
            # Перепроверка ответа после repair (не даём repair сломать ответ).
            _re_verify = verify_answer(
                repaired, base_ctx, base_plan,
                condition_text=condition_text, settings=engine.settings,
            )
            if _re_verify.get("verdict") != "mismatch":
                solver_result = repaired
                job.solution_json = _fmt_plan_json(solver_result)
                job.solver_answer = str(
                    (solver_result.get("answer") or {}).get("value", "")
                )
                compiled, issues = compile_solver_aux(solver_result, base_plan)
                fidelity = fidelity_report(solver_result, base_plan)
                job.aux_fidelity = fidelity.get("ratio")
                aux_source = "solver_repair"
                logger.info(
                    "[figures_gen] aux_repair job %d: dropped after repair = %d",
                    job_id, fidelity.get("dropped", 0),
                )
            else:
                logger.warning(
                    "[figures_gen] aux_repair rejected (answer mismatch) job %d",
                    job_id,
                )

    if issues:
        logger.warning("[figures_gen] solver aux compile issues (final) job %d: %s",
                       job_id, issues)
    ...
    # CH-fidelity (жёсткое правило): если solver явно продиктовал доп. построения
    # и компилятор их скомпилировал без потерь (fidelity >= 1.0), то эти
    # построения — ИСТОЧНИК ИСТИНЫ.  Численная эвристика полезности может ложно
    # дать «useless» (например, когда построение не даёт новых числовых
    # равенств, хотя геометрически необходимо), из-за чего чертёж молча
    # откатывался к базовому.  Теперь: при fidelity>=1.0 НИКОГДА не откатываем;
    # usefulness оставляем только как телеметрию.
    declared = fidelity.get("declared", 0)
    fid_ok = fidelity.get("ok") and declared > 0
    if not usefulness.get("useful") and not fid_ok:
    ...
    if hard_errs:
        # CH-fidelity (жёсткое правило): если solver продиктовал построения,
        # компилятор скомпилировал их без потерь (fidelity>=1.0) и движок их
        # успешно отрисовал (aux_svg непуст), то визуальный аудит — это
        # ТЕЛЕМЕТРИЯ, а не повод откатывать чертёж.  Раньше любая HARD-ошибка
        # (например, ложно-положительный MISSING_LABEL / LINE_OUT_OF_CANVAS на
        # сложной incircle-цепочке) молча убирала ВСЕ доп. построения.
        # Откатываем только если fid НЕ ok (значит, компилятор потерял часть).
        if not fid_ok:
            job.aux_fail_reason = _fmt_plan_json({
            ...
            _charge_credit(job_id)
            return
        # fid ok: логируем предупреждение, но НЕ откатываем.
        job.aux_reason = (job.aux_reason or "") + \
            f" [visual_warnings: {len(hard_errs)}]"
        db.session.commit()
            ...
            with app.app_context():
                ...
                stale = FigureBuildJob.query.filter(
                    FigureBuildJob.status.in_([
                        ...
                        'aux_compile', 'aux_repair', 'aux_usefulness',
                    ]),
...
def _stage_label(stage: str) -> str:
    ...
    labels = {
        ...
        "aux_repair": "Уточнение построений",
