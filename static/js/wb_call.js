...
(function () {
  ...
  // Список «мельтешащих» промежуточных статусов, которые пользователь
  // видеть не должен — они появляются и сразу перекрываются следующим,
  // создавая ощущение, что что-то «дёргается». Если хочется их вернуть
  // для отладки — поставить window.WB_CALL_VERBOSE = true в консоли.
  var _NOISY_STATUSES = {
    "Запрашиваем камеру…": 1,
    "Подключаемся к комнате…": 1,
    "Соединяемся…": 1,
    "Соединяемся с собеседником…": 1,
    "Собеседник подключился…": 1,
    "Перепоодключаемся…": 1,         // (старое написание — оставлено для совместимости)
    "Переподключаемся…": 1,
    "Соединение восстановлено": 1,
    "В разговоре · доска синхр.": 1,  // «В разговоре» и так показывается
    "Перезапрашиваем камеру/микрофон…": 1,
    "Камера/микрофон обновлены": 1,
    "Подключён только звук": 1,
    "Камера недоступна — подключаемся только со звуком…": 1,
    "Ссылка скопирована": 1,
    "Приглашение отправлено": 1
  };

  function setStatus(text, kind) {
    // Тихо игнорируем «шумные» промежуточные сообщения. Ошибки (kind="err")
    // и важные предупреждения проходят всегда.
    if (
      text &&
      kind !== "err" &&
      _NOISY_STATUSES[text] &&
      !window.WB_CALL_VERBOSE
    ) {
      return;
    }
    var el = $("wbCallStatus");
    ...
    fetch("/api/wb_call/invite", {
      ...
      try {
        ...
        if (callCode) {
          ...
          // Маленькая задержка, чтобы пользователь увидел «[OK] Приглашён».
          setTimeout(function () {
