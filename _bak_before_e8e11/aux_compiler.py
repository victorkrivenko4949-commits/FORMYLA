...
def _recognize_incircle(aux_cs: List[dict], base_plan: Dict) -> Optional[List[dict]]:
    ...
    # иначе создаём нативно под уникальным именем aux_O.
    ...
    for side_label, opp, s1, s2, existing_names, fallback_id in touch_defs:
        ...
        for name in existing_names:
            if name in base_ids:
                ...
                break
        if touch_id is None:
            touch_id = fallback_id
            constructions.append({
                "type": "incircle_touch", "id": touch_id,
                # ВАЖНО: geom.incircle_touch_point(a, b, c) трактует a как
                # ПРОТИВОЛЕЖАЩУЮ вершину, а (b, c) — концы стороны касания.
                # Движок incircle_touch пробрасывает (p1, p2, p3) напрямую,
                # поэтому p1 = противоположная вершина (opp), p2/p3 = концы.
                # Раньше был порядок (s1, s2, opp) — точка касания попадала
                # на ДРУГУЮ сторону, и вписанная окружность ломалась.
                "p1": opp, "p2": s1, "p3": s2,
                "label": "", "side": "auto",
...
def compile_solver_aux(solver_result: dict, base_plan: dict) -> Tuple[dict, List[str]]:
    ...
    for i, ac in enumerate(aux_cs):
        ...
        if op in ("segment", "line", "ray"):
            ...
            pid = ac.get("id") or ac.get("point") or ""
            ...
            if len(seg) >= 2 and len_m and ext_id:
                ...
                # «AD = AM»: расстояние = |AM|.  length_from = вторая точка
                # правой части (M) — движок возьмёт dist(origin, M) = |AM|.
                c["length_from"] = len_m.group(4)
                c["id"] = ext_id
                created.add(ext_id)
            elif len(seg) >= 2:
                p1 = seg[0]
...
def fidelity_report(solver_result: dict, base_plan: dict) -> Dict[str, Any]:
    ...
    # DUPLICATE_IN_BASE — НЕ потеря: объект уже существует в base, повторно
    # строить его не нужно.  Такое построение можно безболезненно пропустить,
    # и repair запускать не обязательно.
    real_issues = [i for i in issues if not i.startswith("DUPLICATE_IN_BASE")]
