...
def make_prompt(job):
    if job['mode'] == 'fill_missing':
        return f"""Создай новую задачу.
Класс: {job['grade']}
Уровень: L{job['difficulty']}
Тема: {job['theme']}
Подтема: {job['subtopic']}
method_code: {job['method_code']}
method: {job['method_code']}
Требование к уровню: {job.get('quality_target','соответствие уровню')}.
Нужен оригинальный task_text, correct_answer и полное solution.
Верни JSON-объект задачи."""
    return f"""Замени плохую задачу новой корректной задачей с тем же id.
id: {job['id']}
Класс: {job['grade']}
Уровень: L{job['difficulty']}
Тема: {job['theme']}
Подтема: {job['subtopic']}
method_code: {job.get('method_code')}
Причины замены: {job.get('reasons')}
Сохрани тот же id. Верни JSON-объект задачи."""
