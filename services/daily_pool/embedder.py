...
def check_deduplication(
    ...
    logger.info(
        f"[Embedder] {olympiad_slug}/{grade}/{round_name} "
        f"max_sim={result['max_similarity']:.3f} dup={result['is_duplicate']}"
    )
    return result
...
def save_embedding(
    ...
    db.session.commit()
    logger.info(f'[Embedder] Saved embedding for problem {problem_id}')
