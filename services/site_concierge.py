...
_ROUTER_SYSTEM_PROMPT_TEMPLATE = """Ты — диспетчер AI-помощника на сайте FORMYLA.net, российской онлайн-платформе подготовки к математическим олимпиадам (ВсОШ, Турнир городов, Эйлера, Ломоносова, Высшая проба, Матпраздник).
...
1) Если вопрос пользователя — про навигацию по сайту FORMYLA.net и явно ложится в один из intent-ов выше:
   {{"action": "kb", "id": "<id_intent>"}}

2) Если вопрос — НЕ про сайт FORMYLA.net:
...
3) Если вопрос — про сайт FORMYLA.net, но НЕ покрывается ни одним intent-ом
...
ОГРАНИЧЕНИЯ для action=free:
   • Не выдумывай цены, фичи, обещания. Опирайся только на факты:
     – Все функции FORMYLA.net сейчас полностью бесплатны — подписок и тарифов нет.
...
_REDIRECT_TUTOR_ANSWER = (
    ...
    "Я помогаю только по сайту FORMYLA.net."
)
_OFFTOPIC_ANSWER = (
    "Я помощник по сайту FORMYLA.net: тарифы, разделы, как начать, как готовиться к олимпиадам. "
...
def build_site_context_for_llm() -> str:
    ...
    Содержит: все разделы/функции FORMYLA.net с реальными URL и фактами
    ...
    kb = _load_kb()
    lines = [
        "ПОЛНАЯ СПРАВКА О САЙТЕ FORMYLA.net (единственный источник правды о сайте):",
        "",
        "Функции и разделы:",
    ]
    for entry in kb:
    ...
    lines += [
        ...
        "  • Все функции FORMYLA.net сейчас полностью бесплатны — подписок, тарифов и оплат нет.",
        "  • 7 ИИ-агентов: алгебра, геометрия, теория чисел, комбинаторика, движение, логика, стратегия.",
        ...
        "честно скажи, что такого в FORMYLA.net нет, и не выдумывай.",
    ]
    return "\n".join(lines)
