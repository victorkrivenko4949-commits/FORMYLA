...
ОБЯЗАТЕЛЬНО СОХРАНИТЬ:
  [OK] Математическую идею и метод решения — ТАКОЙ ЖЕ
  [OK] Сложность — ТАКУЮ ЖЕ
  [OK] Жанр — ТАКОЙ ЖЕ
  [OK] Тип ответа — ТАКОЙ ЖЕ (целый -> целый, etc.)

ОБЯЗАТЕЛЬНО ИЗМЕНИТЬ:
  [ERROR] Все числа — ДРУГИЕ (но ответ остаётся красивым)
  [ERROR] Имена персонажей — ДРУГИЕ
  [ERROR] Контекст/сюжет — ПОЛНОСТЬЮ ДРУГОЙ
  [ERROR] Формулировку — ПЕРЕПИСАТЬ своими словами
  [ERROR] Задача НЕ ДОЛЖНА гуглиться —
...
БЕЗ ИСКЛЮЧЕНИЙ — даже одиночная переменная:
  [ERROR] "переменная x равна 5"
  [OK] "переменная $x$ равна $5$"
...
СИМВОЛЫ — ТОЛЬКО LaTeX-команды (НЕ юникод!):
  [ERROR] ∠ABC = 60°  ≠  ≡  △ABC  √45  ≤  ≥
  [OK] $\\angle ABC = 60^\\circ$
  [OK] $\\neq$, $\\equiv$, $\\triangle ABC$
  [OK] $\\sqrt{{45}}$, $\\leq$, $\\geq$

ДРОБИ:
  [ERROR] 1/2, a/b
  [OK] $\\frac{{1}}{{2}}$, $\\frac{{a}}{{b}}$

КОРНИ (ВАЖНО — частая ошибка!):
  [ERROR] √45, корень из 45, \\sqrta, \\sqrt a, \\sqrtab
  [OK] $\\sqrt{{45}}$, $\\sqrt{{a}}$, $\\sqrt{{a+b}}$
...
1. НЕ ВКЛЮЧАЙ В УСЛОВИЕ:
   [ERROR] Решение, ответ, подсказки
   [ERROR] Слова "Решение:", "Доказательство:", "Ответ:"
...
def generate_olympiad_task(
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if result['valid']:
            ...
            logger.info(
                f"[OK] Задача сгенерирована за {attempt} попытку(и): "
    ...
    logger.error(
        f"[ERROR] Не удалось сгенерировать задачу за {max_retries} попытки: "
