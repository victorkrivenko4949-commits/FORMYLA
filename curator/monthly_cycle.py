...
def get_cycle_info(user_id: int) -> Dict[str, Any]:
    ...
    if n == 0:
        ...
        # Срез УБРАН (2026-09): раньше задачи дня первую неделю блокировались
        # до прохождения утреннего среза по теме — это оказалось главным
        # обрывом воронки (новчики массово не доходили до продуктового ядра).
        # Теперь задачи дня доступны сразу после анкеты; уровень берётся
        # из анкеты (prior_mu), а тематическая калибровка достраивается
        # фоном через running_pct по решённым задачам.
