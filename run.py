...
ЗАПУСК:
    python -u run.py            # аудит, затем фикс
    python -u run.py audit
    python -u run.py fix
    python -u run.py stats
"""
...
WORKERS = 5           # при ошибках 429 снизь до 3
MAX_TOKENS = 64000
TIMEOUT = 1200
MAX_RETRIES = 3
FIX_CYCLES = 5
...
def stage_stats():
    ...
    print(f"Аудировано   : {len(ok)} (ошибок {len(aud) - len(ok)})")
