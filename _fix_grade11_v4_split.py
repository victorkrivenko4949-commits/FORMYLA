...
# -*- coding: utf-8 -*-
"""
Fix grade 11 vsosh 2020 regional Day 2 problems by splitting into 2 API calls.
Call 1: problems 6-7 (max_tokens=16000)
Call 2: problems 8-10 (max_tokens=16000)
This avoids the truncation issue seen with all 5 problems in one call.
"""
import ast
import json
import os
...
def save_olympiads_db(db_list):
    ...
    if os.path.exists('olympiads.py'):
        shutil.copy2('olympiads.py', 'olympiads_backup_grade11_v4.py')
        print("  Backup: olympiads_backup_grade11_v4.py", flush=True)
...
def main():
    ...
    # Call 1: problems 6-7 (2 problems - smaller, easier to fit)
...
Верни ТОЛЬКО валидный JSON-массив из 2 объектов (задачи 6 и 7). Каждый объект должен иметь поля:
  ...
  - "solution": краткое решение (3-5 предложений, не более 500 символов)
...
[
  ...
  {{
    ...
    # Save raw response
    with open('_last_response_11_v4_call1.txt', 'w', encoding='utf-8') as f:
        f.write(response_67)
    print(f"  Call 1 response saved to _last_response_11_v4_call1.txt ({len(response_67)} chars)", flush=True)
    print(f"  Preview: {response_67[:200]}...", flush=True)
...
[
  ...
  {{
    ...
    # Save raw response
    with open('_last_response_11_v4_call2.txt', 'w', encoding='utf-8') as f:
        f.write(response_810)
    print(f"  Call 2 response saved to _last_response_11_v4_call2.txt ({len(response_810)} chars)", flush=True)
    print(f"  Preview: {response_810[:200]}...", flush=True)
    ...
    if len(valid) < 3:
        print(f"  ERROR: Too few valid problems ({len(valid)}), cannot proceed")
        # Try fallback: just take all problems with text
        print("  Trying fallback: using problems with text regardless of num...")
        valid = []
        for p in all_day2_problems:
    ...
    if os.path.exists(db_py_path):
        ...
        shutil.copy2(db_py_path, db_py_path + '.bak4')
        with open(db_py_path, 'w', encoding='utf-8') as f:
            f.write(db_content)
        print(f"  Updated {db_py_path} ({len(db_content)} bytes)", flush=True)
    
    print("\n[OK] Grade 11 Day 2 problems added successfully using split approach!")
