...
======================================
max_tokens=16000 (место для ответа после рассуждений)
...
def api_call(messages, max_tokens=16000, timeout=300):
    payload = {
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "error" not in d:
                ...
                # Если content пустой — берём из reasoning
                if not content and reasoning:
                    # Ищем JSON в reasoning
                    return reasoning
                return content
...
def main():
    print('=' * 60)
    print('FORMYLA L4-L5: РИЗОНЕР (max_tokens=16000, отладка)')
    print('=' * 60)
    ...
    print(f'max_tokens=16000, thinking=enabled, 1 поток')
    print()
