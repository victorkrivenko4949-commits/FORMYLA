...
def test_free_user_blocked_after_3_ai_explanations(service, sub_test_user, db):
    ...
    can_use, msg = service.can_use_feature(sub_test_user, 'ai_explanation')

    assert can_use is False
    assert '3' in msg or 'лимит' in msg.lower() or 'тариф' in msg.lower()


def test_free_user_not_blocked_at_2_ai(service, sub_test_user, db):
    today = datetime.utcnow().date().isoformat()
    db.execute("""
        INSERT OR REPLACE INTO usage_daily (user_id, date, ai_explanations_used)
        VALUES (?, ?, 2)
    ...
    can_use, _ = service.can_use_feature(sub_test_user, 'ai_explanation')
    assert can_use is True


def test_free_user_blocked_after_20_tasks(service, sub_test_user, db):
    today = datetime.utcnow().date().isoformat()
    db.execute("""
    ...
    can_use, msg = service.can_use_feature(sub_test_user, 'task')
    assert can_use is False
    assert '20' in msg or 'лимит' in msg.lower()


def test_free_user_can_use_task_initially(service, sub_test_user):
    can_use, msg = service.can_use_feature(sub_test_user, 'task')
    assert can_use is True
    assert msg == ''


def test_free_user_can_use_ai_initially(service, sub_test_user):
    can_use, _ = service.can_use_feature(sub_test_user, 'ai_explanation')
    assert can_use is True
...
def test_activate_premium_sets_max_tokens(service, sub_test_user):
    service.activate_premium(sub_test_user, plan='premium_monthly', is_beta=True)
    plan = service.get_user_plan(sub_test_user)
    assert plan['ai_max_tokens'] == 8000


def test_activate_yearly_premium(service, sub_test_user):
    result = service.activate_premium(sub_test_user, plan='premium_yearly', is_beta=True)
...
def test_premium_unlimited_ai(service, sub_premium_user, db):
    ...
    db.execute("""
        INSERT OR REPLACE INTO usage_daily (user_id, date, ai_explanations_used)
        VALUES (?, ?, 10)
    """, (sub_premium_user, today))
    db.commit()

    can_use, msg = service.can_use_feature(sub_premium_user, 'ai_explanation')
    assert can_use is True


def test_premium_unlimited_tasks(service, sub_premium_user, db):
    today = datetime.utcnow().date().isoformat()
    db.execute("""
        INSERT OR REPLACE INTO usage_daily (user_id, date, tasks_completed)
        VALUES (?, ?, 100)
    ...
    can_use, _ = service.can_use_feature(sub_premium_user, 'task')
    assert can_use is True
...
def test_premium_at_199_not_blocked(service, sub_premium_user, db):
    today = datetime.utcnow().date().isoformat()
    db.execute("""
        INSERT OR REPLACE INTO usage_daily (user_id, date, ai_explanations_used)
        VALUES (?, ?, 199)
    ...
    can_use, _ = service.can_use_feature(sub_premium_user, 'ai_explanation')
    assert can_use is True
...
def test_active_premium_not_expired(service, sub_test_user, db):
    future_at = (datetime.utcnow() + timedelta(days=30)).isoformat()
    db.execute("""
        INSERT INTO subscriptions
            (user_id, plan, status, expires_at, is_beta_access)
        VALUES (?, 'premium_monthly', 'active', ?, 1)
    """, (sub_test_user, future_at))
    db.commit()
...
def test_increment_task_usage(service, sub_test_user):
    """Test 7: счётчики правильно увеличиваются."""
    service.increment_usage(sub_test_user, 'task')
    usage = service.get_today_usage(sub_test_user)
    assert usage['tasks_completed'] == 1


def test_increment_ai_usage(service, sub_test_user):
    service.increment_usage(sub_test_user, 'ai_explanation', tokens=500, cost_usd=0.001)
    usage = service.get_today_usage(sub_test_user)
    assert usage['ai_explanations_used'] == 1
    assert usage['tokens_consumed'] == 500


def test_increment_multiple_times(service, sub_test_user):
    for _ in range(5):
        service.increment_usage(sub_test_user, 'task')
    usage = service.get_today_usage(sub_test_user)
...
def test_get_today_usage_empty(service, sub_test_user):
    usage = service.get_today_usage(sub_test_user)
...
def test_free_max_tokens(service, sub_test_user):
    """Test 8: Free получает 2000."""
    plan = service.get_user_plan(sub_test_user)
    assert plan['ai_max_tokens'] == 2000


def test_premium_max_tokens(service, sub_test_user):
...
def test_yearly_premium_max_tokens(service, sub_test_user):
    service.activate_premium(sub_test_user, plan='premium_yearly', is_beta=True)
    plan = service.get_user_plan(sub_test_user)
    assert plan['ai_max_tokens'] == 8000
...
def test_cancel_active_subscription(service, sub_test_user, db):
    service.activate_premium(sub_test_user, plan='premium_monthly', is_beta=True)

    result = service.cancel_subscription(sub_test_user)
...
def test_cancel_no_subscription(service, sub_test_user):
    result = service.cancel_subscription(sub_test_user)
...
def test_free_user_display(service, sub_test_user, db):
    today = datetime.utcnow().date().isoformat()
    db.execute("""
    ...
    display = service.get_usage_for_display(sub_test_user)
...
def test_premium_user_display_no_daily_limit(service, sub_premium_user, db):
    today = datetime.utcnow().date().isoformat()
    db.execute("""
        INSERT OR REPLACE INTO usage_daily
            (user_id, date, tasks_completed, ai_explanations_used)
    ...
    display = service.get_usage_for_display(sub_premium_user)
